...
* MENU OPTION E
 ...
 If user chooses menu option "e", the system allows you to return to the start screen of the program.
...
| 4.	Appointments could be made for file numbers who didn't have patient details stored in system yet. This would then cause problems when user checks for patient details of an appointment that is made as there would be none.It would be considered as a logical error in the real world. You always need a patient's details before booking an appointment for them. | This is solved by first checking if file number entered by user to make appointment was in the patients sheet first, if that specific file number is already stored in the patients sheet then program proceeds to ask user to enter appointment date and time to make a booking. However, If inputted file number does not correspond to a file number in patient's sheet then user will be redirected to the main menu to enter patient's details before making an appointment. For example: user enters file no #78653 to make appointment and the system will check through patients sheet file number column ![patient sheet](documentation/bugs/patient-details-sheet.png) . As you can see the specific file number is in patient sheet file number column, and therefore will proceed to ask user with the necessary questions to make an appointment. If that file number is not found in patient's sheet file number column, user will be redirected like so -![Add patient](documentation/bugs/no-details-bug.png) |
| 5.	When user tried to view a patients appointments, User is asked to input patient's file number. This gave error because appointment was not yet made with the specific patient yet . | Solved by using for loop to check if inputted file number was matching with a file number in the file no column appointment sheet in database/gspread and if it wasn't there then user is returned back to menu.![No appointment](documentation/bugs/no-appointment-error.png)  |
...
1. I wanted the background image to always have the terminal placed in a specific position. I know that the terminal position changes with each device. Ideally the terminal should placed where the computer screen is in the picture to give the effect of user being infront of a desk in a private practice's reception. I did not try too hard to solve this problem because of time constraints and because the assesment is soley focused on python, not css or html.
